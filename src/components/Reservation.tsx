...
export default function Reservation() {
  const { phone, onlineUrl, hours, note } = storeData.reservation
  ...
  return (
    <section id="reservation" className="py-20 bg-white">
      <div className="max-w-3xl mx-auto px-4">
        ...
        <div className={`grid ${onlineUrl ? 'sm:grid-cols-2' : 'sm:grid-cols-1 max-w-sm mx-auto'} gap-6 mb-8`}>
          <div className="bg-warm-50 rounded-lg p-6 border border-stone-100 flex flex-col items-center text-center">
            ...
            {hours && (
              <p className="text-sm text-stone-500">受付時間：{hours}</p>
            )}
          </div>
          ...
          {onlineUrl && (
            <div className="bg-warm-50 rounded-lg p-6 border border-stone-100 flex flex-col items-center text-center">
              ...
              <a href={onlineUrl} className="btn-primary text-sm w-full">
                ネット予約はこちら
              </a>
            </div>
          )}
